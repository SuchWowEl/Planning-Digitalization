<div class="border-2 border-white h-full w-full bg-white text-black rounded-md p-6 shadow-lg">
    ...
    @foreach ($details_string as $name => $title)
        ...
        <input type="text" id="{{ $name }}" name="details[{{ $name }}]" class="mb-6 min-h-9 border-2 w-full outline-none focus:border-gray-300 transition col-span-2 px-2 rounded-md">
    @endforeach
    ...
    <div class="grid grid-cols-12 gap-1 mb-6" x-data="{ btotal: 0, add_mf() {
        ...
        <div class="md:col-span-7 col-span-6">
            ...
            <input type="text" id="type" name="details[type]" class="min-h-9 border-2 w-full outline-none focus:border-gray-300 transition col-span-2 px-2 rounded-md"></input>
        </div>
        @php $genders = ["male", "female"]; @endphp
        @foreach ($genders as $g)
            <div class="col-span-2">
                <label for="{{ $g }}" class="w-full">{{ ucfirst($g) }}:</label>
                <input @blur="add_mf()" placeholder="0" min="0" type="number" id="{{ $g }}" x-ref="{{ $g }}" name="details[{{ $g }}]" class="min-h-9 border-2 w-full outline-none focus:border-gray-300 transition col-span-2 px-2 rounded-md" required></input>
            </div>
        @endforeach
        <div class="md:col-span-1 col-span-2">
            <label class="w-full">Total:</label>
            <div name="details[total]" class="min-h-9 w-full outline-none transition col-span-2 px-2 border-2 inline-block rounded-md cursor-not-allowed" x-text="btotal"></div>
        </div>
    </div>

    <div class="w-full flex flex-col mb-6">
        <label class="w-full">7. Implementation Period</label>
        <template x-if="ppa_stat == '25' || ppa_stat == ''">
            <div class="w-full">
                <div class="flex flex-row justify-center text-wrap">
                    N/A since PPA is routine/continuing (or PPA Status hasn't been set yet)
                </div>
                <input x-ref="impl1" type="date" id="start" name="details[start]" value="none" class="hidden min-h-9 border-2 w-full outline-none focus:border-gray-300 transition px-2 rounded-md"></input>
                <input x-ref="impl2" type="date" id="end" name="details[end]" value="none" class="hidden min-h-9 border-2 w-full outline-none focus:border-gray-300 transition px-2 rounded-md"></input>
            </div>
        </template>
        <template x-if="ppa_stat != '25' && ppa_stat != ''"">
            <div class="grid grid-cols-6 gap-1">
                <div class="md:col-start-2 md:col-span-2 col-span-3">
                    ...
                    <input x-ref="impl1" type="date" id="start" name="details[start]" class="min-h-9 border-2 w-full outline-none focus:border-gray-300 transition px-2 rounded-md"></input>
                </div>
                <div class="md:col-start-4 md:col-span-2 col-span-3">
                    <label for="end">End:</label>
                    <input x-ref="impl2" type="date" id="end" name="details[end]" class="min-h-9 border-2 w-full outline-none focus:border-gray-300 transition px-2 rounded-md"></input>
                </div>
            </div>
        </template>
    </div>
    ...
    <div class="grid grid-cols-6 gap-1 mb-6">
        ...
        x-init="
            $nextTick( () =>
                {
                    let value = 0;
                    let array = Object.keys(ind);
                    value = Math.max.apply(null, array);
                    console.log(value);
                    ind_last_count = ++value;
                }
            )
        "
        x-data="{
    ...
    <div class="grid grid-cols-7 gap-1 mb-6 mx-8 p-2 border-2 rounded-md  lg:divide-y-0 divide-y lg:divide-x divide-x-0 bg-slate-100">
        <div class="lg:col-span-3 lg:grid lg:grid-cols-12 col-span-full flex flex-col w-full">
            <div class="gap-1 col-span-full">
                <input id="gfp" type="checkbox" name="details[fund][]" value="General Fund Proper" x-model="sof">
            ...
            <div class="grid grid-cols-12 col-span-full justify-between">
                <template x-if="sof.includes('General Fund Proper')">
                    <div class="gap-1 col-start-2 lg:col-start-3 w-[120px]">
                        <input id="gf" type="checkbox" name="details[subfund][]" value="GF - Proper" x-model="ssof"></input>
                    ...
                    <template x-if="sof.includes('General Fund Proper')">
                        ...
                        @foreach ($fund_gfp as $name)
                            ...
                            <div class="gap-1 lg:col-start-2 lg:col-span-8 col-span-full">
                                <input id="{{ $name }}" type="checkbox" name="details[subfund][]" value="{{ $name }}" x-model="ssof"></input>
        ...
        <div class="lg:col-span-2 grid grid-cols-12 col-span-full px-1">
            <div class="gap-1 col-span-full h-min">
                <input id="trust_f" type="checkbox" name="details[fund][]" value="Trust Fund" x-model="sof"></input>
            ...
            <div class="col-start-2 col-span-full">
                <template x-if="sof.includes('Trust Fund')">
                    <div class="w-full">
        ...
        <div class="lg:col-span-2 col-span-full px-1">
            <div class="gap-1">
                <input id="sef" type="checkbox" name="details[fund][]" value="Special Education Fund" x-model="ssof"></input>
